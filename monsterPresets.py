...
class medusaPreset:
    ...
    def battleTatic(self, rpg, player):
        if self.firstTurn == True:
            ...
            if self.monster.stats['Hp'] < round(self.monster.stats['Max Hp'] * .90) or self.monster.stats['Stamina'] < round(self.monster.stats['Max Stamina'] * .45) or self.monster.stats['Mana'] < round(self.monster.stats['Max Mana'] * .45): 
                rest(self.monster)
            else:
                rpg.combatGUI.showEnemySlot2Indicator = False
                attack(rpg, self.monster, player)
